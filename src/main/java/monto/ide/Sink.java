...
public class Sink {
    ...
    public <A,E extends Exception> A receive(PartialFunction<ProductMessage, A, E> onProductMessages, PartialFunction<DiscoveryResponse, A, E> onDiscovery) throws UnrecongizedMessageException, MessageUnavailableException, E {
		String rawMsg = socket.recvStr();
        if (rawMsg != null) {
        	return GsonMonto.fromJson(rawMsg, IDEReceive.class).<A,E>match(
        			prod -> onProductMessages.apply(prod),
        			disc -> onDiscovery.apply(disc));
        } else {
